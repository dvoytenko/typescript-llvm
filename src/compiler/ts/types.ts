...
function logType(tsType: ts.Type, {checker}: CompilerContext) {
  ...
  console.log('QQQQ: TYPE FLAGS: ', {
    ...
    symbol: tsType.getSymbol() ? checker.symbolToString(tsType.getSymbol()!) : null,
    // toString, toFixed, ...
    properties: tsType.getProperties().map(prop => checker.symbolToString(prop)),
    // toString, toFixed, ...
    apparentProperties: tsType.getApparentProperties().map(prop => checker.symbolToString(prop)),
    // []
    callSignatures: tsType.getCallSignatures().map(sig => checker.signatureToString(sig)),
    // []
    constructSignatures: tsType.getConstructSignatures().map(sig => checker.signatureToString(sig)),
    // null
    stringIndexType: tsType.getStringIndexType() ? checker.typeToString(tsType.getStringIndexType()!) : null,
    // null
    numberIndexType: tsType.getNumberIndexType() ? checker.typeToString(tsType.getNumberIndexType()!) : null,
    // null
    baseTypes: tsType.getBaseTypes() ? tsType.getBaseTypes()!.map(t => checker.typeToString(t)) : null,
    // number
    nonNullableType: checker.typeToString(tsType.getNonNullableType()),
    // null
    constraint: tsType.getConstraint() ? checker.typeToString(tsType.getConstraint()!) : null,
    // null
    def: tsType.getDefault() ? checker.typeToString(tsType.getDefault()!) : null,
